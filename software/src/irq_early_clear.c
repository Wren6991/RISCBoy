...
// Each handler clears its own IRQ and then next one.
// Only even-numbered IRQs should fire, since odd-numbered clear
// before we enter their handler.
#define N_IRQS 16

volatile int irq_order[N_IRQS];
volatile int n_irqs_received;

#define ACK_IRQ(irq) ISR(irq)() {*TBMAN_IRQ_FORCE &= ~(3ul << irq);\
...
int main()
{
	...
	*TBMAN_IRQ_FORCE = ~0u;
	...
	while (n_irqs_received < N_IRQS / 2)
		;

	tbman_puts("IRQs received. Checking order\n");
	bool failed = false;
	for (int i = 0; i < N_IRQS / 2; ++i)
	{
		tbman_putint(irq_order[i]);
		failed |= irq_order[i] != 2 * i;
	}
