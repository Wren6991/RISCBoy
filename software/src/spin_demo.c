...
static inline void affine_mul(int32_t result[6], int32_t left[6], int32_t right[6])
{
...
static inline void affine_copy(int32_t dst[6], int32_t src[6])
{
	for (int i = 0; i < 6; ++i)
		dst[i] = src[i];
}
...
// *post*-multiplication.
static inline void affine_identity(int32_t current_trans[6])
{
	int32_t tmp[6] = {
		AF_ONE, 0,      0,
		0,      AF_ONE, 0
...
static inline void affine_translate(int32_t current_trans[6], int32_t x, int32_t y)
{
	int32_t tmp[6];
	int32_t transform[6] = {
		AF_ONE, 0,      -AF_ONE * x,
