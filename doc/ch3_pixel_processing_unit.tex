...
\begin{bytefield}[endianness=big,bitformatting=\tiny]{32}
\bitheader{0,11,12,23,24,31} \\
\bitbox{8}{{\tt JUMP}} \bitbox{12}{{\tt x}} \bitbox{12}{{\tt y}} \\
\bitheader{0, 31} \\
\bitbox{32}{{\tt addr}} \\
\end{bytefield}

If the current raster position matches ({\tt x}, {\tt y}), then jump to address {\tt addr}. Otherwise, fall through to the next instruction. The match condition for ({\tt x}, {\tt y}) is the same as for {\tt WAIT}: all-ones will match any position on that axis.
...
\bitbox{8}{{\tt POKE}} \bitbox{12}{ } \bitbox{12}{{\tt addr}} \\
\bitheader{0, 31} \\
\bitbox{32}{{\tt data}} \\
\end{bytefield}
...
Suppose you want to draw a menu on one side of the screen, but you have already used up all your background layers to render the game world. In this case you would {\tt POKE} a background tileset and tilemap at the same point on each scanline, so that the left half of the screen shows the game world, and the right half of the screen shows some bitmap font. You would also {\tt POKE} the sprites to disable them, so that they don't draw over the menu. The PPU is paused while the Poker is running, so all of these changes take effect between pixel {\tt x} of the most recent {\tt WAIT} instruction, and pixel $\mathtt{x} + 1$.
