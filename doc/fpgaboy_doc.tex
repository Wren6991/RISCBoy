...
Hazard5 is a 32-bit processor based on the RISC-V instruction set architecture. Targeting a small FPGA, Hazard5 sits close to the embedded design space -- RV32IC is an austere instruction set. It does make some concessions to performance, such as a 5-stage pipeline with full bypass.

People familiar with the textbook 5-stage RISC pipeline will find Hazard5 mostly straightforward. We will use the following symbols to refer to the 5 stages:
...
The front end consists of stage \texttt{F} and part of stage \texttt{W}. Its purpose is to feed \texttt{D} with instructions, whilst meeting the following constraints:
...
\begin{itemize}
	...
	\item No performance penalty for unaligned jump to 16-bit instruction; ex
	\item Attempt to maintain performance when competing with the load/store unit and AHBL busmaster peers, without relying on caches
\end{itemize}

A RISC-V instruction stream is not naturally aligned, by which we mean instruction address modulo instruction size is not always zero. The front end spends buffering and logic on the common case of sequential execution.


\subsubsection{Program Counter}

Hazard5 does \textit{not} use the program counter (\texttt{PC}) for code fetching, during sequential execution. \texttt{PC} is used exclusively for the link value in \texttt{JAL(R)}, mispredict recovery, and therefore lives in \texttt{D}.
