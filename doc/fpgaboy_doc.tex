...
The encoder logic used in the cache lookup does not give correct output if a tag appears multiple times in the cache (however, it is smaller/faster as a result). It is therefore vital that the same piece of data is not written more than once. There is a behaviour pattern here that can put \texttt{ICACHE} into an invalid state if we fetch from the same address on consecutive cycles:

\begin{enumerate}
	\item Miss on first cycle; \texttt{W} performs an AHB address phase.
	\item \texttt{W} performs duplicate AHB address phase; writeback value is not yet visible in cache.
	\item One copy of data has been written to \texttt{ICACHE}; another write is in progress. \texttt{W} hits \texttt{ICACHE} successfully on this cycle.
	\item The second write has put \texttt{ICACHE} into an invalid state, and all subsequent fetches are undefined.
\end{enumerate}

 This does not happen during normal execution: sequential code fetches are always to incrementing addresses, and jumps have a penalty cycle, which hides the \texttt{ICACHE} writeback hazard, so we do not require any data forwarding paths to hide it. However, this hazard does require careful handling when \texttt{D} is stalled on later stages.

The solution is some kind of forwarding path. We could build the forwarding into the \texttt{ICACHE}, but we would like this cache to synthesise with iCE40 BRAMs if possible to save on LUTs, and these BRAMs have synchronous readout; our forwarding needs to be after the clock edge.

Luckily, our \texttt{F} logic is currently very unbalanced: the memory read path has AHB data phase with some mux logic on the end of it, whereas the \texttt{ICACHE} read path has a registered input going into the mux logic. There is plenty of space here to mux the \texttt{ICACHE} readout.

\begin{itemize}
	\item AHB read data and address are registered at the end of every non-stall cycle.
	\item Both \texttt{W} and \texttt{F} look at the registered fetch address:
	\begin{itemize}
		\item \texttt{W} inhibits AHB address phase on match
		\item \texttt{F} muxes registered AHB data onto \texttt{ICACHE} read bus
	\end{itemize}
\end{itemize}
...
Stalling: a stage halts and causes prior stages to not advance their state, until this stage is ready.
...
\begin{itemize}
	\item \texttt{F}
	\begin{itemize}
		\item AHB HREADY low, on a code fetch transaction
		\item Stall in section \texttt{D}, \texttt{X} or \texttt{M}
	\end{itemize}
	\item \texttt{D}
	\begin{itemize}
		\item Stall in \texttt{X} or \texttt{M}
		\item Tries to assert a jump, but \texttt{W} fetch is stalled
	\end{itemize}
	\item \texttt{X}
	\begin{itemize}
		\item \texttt{W} is using the AHB busmaster
		\item Stall in \texttt{M}
		\item \texttt{M} is about to write to a register \texttt{X} depends on
	\end{itemize}
	\item \texttt{M}
	\begin{itemize}
		\item AHB HREADY low, on a load/store transaction.
		\item Tries to assert a jump, but \texttt{W} fetch is stalled
	\end{itemize}
	\item \texttt{W}
	\begin{itemize}
		\item Writeback: never stalls
		\item PC generation: stalls on \texttt{F} stalling
	\end{itemize}
\end{itemize}

\texttt{D} and \texttt{M} stall on jumps, because there is no way of cancelling the AHB transaction currently in flight, so we need to wait to start the new one.

Pipeline bubbles (no-ops) are inserted by a stage if it is stalled, but the next stage is not. It does this by zeroing out the destination register field, memory op field, and any other pipe register fields which cause side effects.
