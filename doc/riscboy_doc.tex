...
\bitbox{8}{\texttt{POKE}} \bitbox{8}{\texttt{count}} \bitbox{16}{\texttt{addr}} \\
\bitheader{0, 31} \\
...
\bitbox{8}{\texttt{SPRITE}} \bitbox{8}{count} \bitbox{8}{\texttt{start}} \bitbox{8}{\texttt{end}} \\
\bitheader{0, 31} \\
\bitbox{32}{\texttt{addr}} \\
\end{bytefield}

Scan through \texttt{count} sprite configuration words starting at \texttt{addr} and assign them to sprites in range \texttt{start}...\texttt{end} if they intersect the current scanline. Disable remaining sprites. Stop scanning if \texttt{end} is reached. If the display has a horizontal blanking period, this is the ideal time to perform \texttt{SPRITE} scanning. This instruction is implemented with hardware already present in the PPU, so presents little extra cost.
...
Alternatively, sprite allocation can be performed by software, and the sprites updated using a \texttt{POKE} instruction. The \texttt{POKE} approach is more flexible, and makes it easier to multiplex hard sprites within a single scanline.

\subsubsection*{SKIP}

\begin{bytefield}[endianness=big,bitformatting=\tiny]{32}
\bitheader{24,31} \\
\bitbox{8}{\texttt{SKIP}} \bitbox{24}{TBD} \\
\end{bytefield}

Skip the following instruction, if the beam has reached some point. Full details TBD. It might be better to provide a conditional jump with a target, because not all of our instructions are the same length (especially \texttt{POKE})
